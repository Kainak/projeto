function fetchPreClube() {
  ...
  fetch(URL, {
      ...
      .then(() => {
          const URL = "https://7ecl58ro.api.sanity.io/v2021-10-21/data/query/production?query=*%5B_type%3D%3D%22horarios_pre_clube%22%5D%0A+%0A&perspective=published";
          fetch(URL, {
            method: "GET",
          })
            .then(result => result.json())
