...
fetch(URLnoticia  , {
  ...
  .then((result) => {
    result.result.forEach((noticia) => {
      ...
      var dataOriginal = noticia.data;
      var [ano, mes, dia] = dataOriginal.split("-");
      var dataInvertida = `${dia}-${mes}-${ano}`;

      var dateTag = document.createElement("p");
      dateTag.innerText = dataInvertida;
      tituloElement.appendChild(dateTag);
    });
  });
